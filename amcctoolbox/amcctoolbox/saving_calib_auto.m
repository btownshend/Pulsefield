...
if exist([ save_name '.mat'])==2,
    ...
    if exist('copyfile'),
        ...
        while cont,
            ...
            save_name_post = [ save_name postfix];
            cont = (exist([ save_name_post '.mat'])==2);
        end;
        copyfile([save_name '.mat'],[save_name_post '.mat']);
        disp(['Copying the current ' save_name '.mat file to ' save_name_post '.mat']);
        if exist([save_name '.m'])==2,
            copyfile([save_name '.m'],[save_name_post '.m']);
            disp(['Copying the current ' save_name '.m file to ' save_name_post '.m']);
        end;
        cont_save = 1;
    else
        disp(['The file currently called ' save_name '.mat is about to be changed.']);
