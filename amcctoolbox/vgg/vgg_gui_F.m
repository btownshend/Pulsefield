function fig=vgg_gui_F(i1,i2,F,flag)
%
...
if strcmp(action,'start'),
   if nargin ~= 3 && nargin ~= 4
      error('Must give 3 or 4 arguments... read the docs.\n');
   end
   ...
   ah1 = axes('Parent', h0, ...
	      'Position',[0 0 .5 1]);
   h1=imshow(i1); hold on; title('Left View');
   set(h1, 'ButtonDownFcn','vgg_gui_F(''b1'');');

   ah2 = axes('Parent',h0, ...
	      'Position',[.5 0 .5 1], ...
	      'Tag','Axes2');
   h2=imshow(i2); hold on; title('Right View');
   set(h2, 'ButtonDownFcn','vgg_gui_F(''b2'');');
   
   if flag == 1
        while row < imgH
           plot(ah1,1:imgW,ones(1,imgW)*row,'r')
           plot(ah2,1:imgW,ones(1,imgW)*row,'r')
           row = row + 50;
        end
   end
...
elseif action(1)=='b' % If the mouse button is pressed:
   % Set which image we pressed the button in
   if action(2)=='1' ud.current=1;
   elseif action(2)=='2' ud.current=2;
   else return;
   end
   % Extract the point that was clicked
   pt=get(ud.ah(ud.current),'CurrentPoint');
   ud.F(:,:,ud.current);
   ...
   % Plot the projected epipolar line from the selected point using the
   % intersection of the line with the image edges
   ud.l=plot(pts(1,:), pts(2,:), [ud.color '-'], ...
	     'LineWidth', ud.size, 'EraseMode','xor');

     % An unfinished attempt to project the epipolar line back to the
     % original image
%    if ud.current==1 
%        ud.current=2;
%    elseif ud.current==2 
...
%    end
%    % Get the two end points of the line
%    pt = [pts(1,1) pts(1,1); pts(1,2) pts(1,2); 1 0]'
%    pts=get_line_points(ud.F(:,:,ud.current)*pt, ud.sizes(1,:));
%    delete(ud.p);
%    delete(ud.l);
%    axes(ud.ah(ud.current));
%    % Plot the selected point
...
% Make sure the points are projected onto the image
