...
function v=calcvisible(p,tgts,doplot)
if nargin<3
  doplot=false;
end
cpos=p.layout.cpos;
...
for l=1:size(lpos,1)
  for c=1:size(cpos,1)
    ...
    if angle>p.camera(c).fov/2
      ...
      ray=pix2m(p.rays.imap,p.rays.raylines{c,l});
      v(c,l)=1;
      for t=1:size(tpos,1)
...
if doplot
  % Plot p.layout
  plot(p.layout.lpos(:,1),p.layout.lpos(:,2),'.','MarkerSize',1);
  plot(p.layout.cpos(:,1),p.layout.cpos(:,2),'og');
  viscircles(tgts.tpos,tgts.tgtdiam/2,'LineWidth',0.5);

  % Plot FOV
  for i=1:size(p.layout.cpos,1)
    ca(1)=atan2(p.layout.cdir(i,2),p.layout.cdir(i,1))+p.camera(i).fov/2;
    ca(2)=atan2(p.layout.cdir(i,2),p.layout.cdir(i,1))-p.camera(i).fov/2;
    for j=1:2
      endpos=p.layout.cpos(i,:)+0.1*[cos(ca(j)),sin(ca(j))];
      plot([p.layout.cpos(i,1),endpos(1)],[p.layout.cpos(i,2),endpos(2)],'k');
    end
  end
  title('calcvisible');
end