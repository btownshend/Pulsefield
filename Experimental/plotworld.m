function plotworld(cdata,world)
if nargin<2
...
for c=1:length(cdata)
  r=cdata(c).r; 
  campos=tw(world,cdata(c).position);
  fprintf('Camera Position: [%.4f,%.4f,%.4f] in grid, [%.4f, %.4f, %.4f] in world\n', cdata(c).position, campos);
  grid=tw(world,r.Xgrid);
  plot3(grid(1,:),grid(2,:),grid(3,:),'.');
  
  for i=[1,cdata(c).target.nX+1,size(grid,2)-cdata(c).target.nY,size(grid,2)]
  ...
  text(campos(1),campos(2),campos(3),sprintf('C%d',cdata(c).camera));
