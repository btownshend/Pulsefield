...
void *Video::runDisplay(void *arg) {
	...
	if (hasEvent) {
	    ...
	    case KeyPress:
		{
			...
			// Intensity pattern toggle
			Lasers::instance()->toggleFlag("intensity");
			world->newMessage() << "Toggled laser intensity pattern";
		    } else if (key==XK_c) {
...
void Video::drawDevice(cairo_t *cr, float left, float top, float width, float height, std::shared_ptr<Laser>laser)  {
	...
	msg2 << std::fixed << "Laser " << (laser->getUnit()+1) << ": " << laser->getPoints().size() << " @ " << std::setprecision(1) << laser->getSpacing()*1000 << " L=" << laser->getLastAllocationLength() << std::setprecision(3);
	if (!laser->isEnabled())
	    msg2 << " (DISABLED)";
