...
public:
	...
	if (host != lo_address_get_hostname(remote)) {
	    ...
	    remote=lo_address_new(host.c_str(),PORT);
	    dbg("Conductor",1) << "Opening connection to " << lo_address_get_url(remote) << std::endl;
	}
    }
	...
	if (lo_send(remote,path.c_str(),"f",value) <0 ) {
	    dbg("Conductor.send",1) << "Failed send of " << path << " to " << lo_address_get_url(remote) << ": " << lo_address_errstr(remote) << std::endl;
	    return -1;
	}
	return 0;
    }
