...
void make_typehint(char* tph, int m, const mxArray *data) {
  ...
  if(! mxIsCell(data)) {
    mexErrMsgTxt("Expecting a cell array for data!");
  }
  ...
  for(i = 0; i < n && i < (m - 1); i++) {
    ...
    case mxDOUBLE_CLASS:
	//      tph[i] = 'd';
	tph[i] = 'f';   // Force to single since some receivers (like MAX) do not support double
      break;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if(sizeof(d) == sizeof(long int)) {
      err = sscanf(strbuf, "osc_address:%lx", (long int *)&d);
  } else if(sizeof(d) == sizeof(long long int)) {
      err = sscanf(strbuf, "osc_address:%llx", (long long int *)&d);
  } else {
