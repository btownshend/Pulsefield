...
% transmitting and receiving variables of type uint8; it can also use Java's
...
%       transmission and back to uint8 when read back into Matlab, so 
...
%    client: jtcp('close',jTcpObj);
%
% e.g., Send/receive uint8 bytes one by one with serialization off. 
...
%    server: jtcp('write',jTcpObj,uint8('Hello client'));
%    client: mssg = jtcp('read',jTcpObj); char(mssg)
%    server: jtcp('write',jTcpObj,uint8('Hello client, read this with helper class'));
...
if exist('port','var')
    if rem(port,1)~=0 || port < 1 || port > 65535
       error([mfilename '.m--Port number must be an integer between 1 and 65535.']);
    end % if
end % if
...
socket.setTcpNoDelay(true);  % Disable Nagle delay
...
if jTcpObj.serialize
    ...
    if ~isa(mssg,'uint8')
        error([mfilename '.m--With ''serialize'' parameter set to false, data to be sent must be of the type ''uint8''.']);
    end
    jTcpObj.socketOutputStream.write(mssg,0,length(mssg));
end

jTcpObj.socketOutputStream.flush;
...
if jTcpObj.serialize
    ...
    if numBytesToRead > numBytesAvailable
        ...
        if doUseHelperClass
            ...
            % call for each byte.
            mssg = zeros(1, numBytesToRead, 'uint8');
            
            for i = 1:numBytesToRead,
                mssg(i) = jTcpObj.socketInputStream.read;
%                fprintf('mssg(%d)=0x%x\n',i,mssg(i));
            end % for
