...
margin=onlev-max(neighlev,offlev);
minmargin=0.2;
for j=1:length(p.camera)
  c=p.camera(j).pixcalib;
  lowmargin=isfinite(c(j).pos(1)) & margin(j,:)<minmargin;
  if sum(lowmargin)>0
...
pause(0.01);
% Set threshold halfway between on and neighbor-on levels
thresh=(onlev+neighlev)/2;
