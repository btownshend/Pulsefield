...
void setup() {
    ...
    if (1) {
        ...
        Serial.print("Stack end(approx) = ");
        Serial.println((int)&i,HEX);
	Serial.print("Total heap space (bytes) = ");
	int space=(int)&i - (int)__malloc_heap_start;
        Serial.println(space);

	// Check max malloc size
	for (i=space;i>0;i-=10) {
	    int *x=(int *)malloc(i);
	    if (x!=NULL) {
	        free(x);
		Serial.print("Max alloc remaining = ");
		Serial.println(i-1);
		break;
	    }
	}
...
void loop() {
	...
	while (client.available() != 0) {
		...
		// Note order is GRB, not RGB
		    ...
		    int cnt=cmdbuf[2]*3;
		    uint8_t *p; // TODO =strip.getPixelAddr(index);
		    //Serial.print("F: ");
		    //Serial.print(index);
		    //Serial.print(",");
		    //Serial.println(cnt);
