...
public class LEDs extends Thread {
	...
	int ledOrder[]; // Map from arduino LED number to sequence here; -1 indicates LED should be
					// blanked
	Boolean outsiders[]; // Outsiders, true if someone present; array maps full circle, starting at
							// logical LED 1
	int bgMode;
		...
		for (int i = 0; i < order.length; i++)
			for (int j = 0; j < ledperstrip; j++) {
				...
				if (j < leadSkip || j > ledperstrip - tailSkip - 1)
					...
					logicalpos = (i * (ledperstrip - leadSkip - tailSkip) + j - leadSkip - offset + nLogical)
							% nLogical;
	...
	void open() {
		...
		try {
			...
			logger.config(
					"Opened socket to " + hostName + ":" + portNumber + " with sendbuf=" + socket.getSendBufferSize());
	...
	int send(byte data[]) {
		...
		try {
			...
			close();
			logger.warning("Unable to send data to arduino socket: " + e.getMessage() + " isconnected="
					+ socket.isConnected() + ", isclosed=" + socket.isClosed());
			return -1;
		}
		flush();
		return 0;
	...
	public void run() {
		...
		while (true) {
			...
			if (time > 10) { // Display message every 10s
				logger.info(String.format("Average LED Update rate = %.2f/sec (nupdates=%d, time=%f)",
						nupdates * 1.0f / time, nupdates, time));
				nupdates = 0;
				start = finish;
			}
		}
	...
	// Set strips for ID
	void stripid() {
		// final String colnames[]={"red","green",
		// "blue","magenta","cyan","yellow","pinkish", "white"};
		final int col[][] = { { 255, 0, 0 }, { 0, 255, 0 }, { 0, 0, 255 }, { 255, 0, 255 }, { 0, 255, 255 },
				{ 255, 255, 0 }, { 255, 64, 64 }, { 255, 255, 255 } };
		...
		for (int k = 0; k < nstrip; k++) {
			...
			// colnames[k],clkpins[k],datapins[k]));
		}
	...
	void pulsebow() {
		...
		for (int i = 0; i < leds.length; i++) {
			...
			for (int j = 0; j < 3; j++) {
				...
				// String.format("t=%.1f, phase=%.0f, amp(1,:)=%.2f %.2f %.2f,lev=%.0f %.0f
				// %.0f",t(1,1),phase(1,1)*180/pi,amp(1,:),lev(1,:));
				v = v | ((int) lev) << (16 - 8 * j);
			}
			leds[i] = v;
		}
