...
class GoalBox {
	...
	void draw(PGraphics g) {
		...
		Visualizer.drawText(g, 0.3f, Integer.toString(score), center.x, center.y + (-1 * outsidey * size));
	}
...
public class VisualizerGravity extends VisualizerParticleSystem {
	...
	VisualizerGravity(PApplet parent) {
		super(parent);
		img = parent.loadImage("circle.png");		
	...
	public void update(PApplet parent, People p) {
		...
		if (universe.particlesRemaining() > particlesPerUpdate) {
			for (int i = 0; i < particlesPerUpdate; i++) {
				...
				PVector corner1Velocity = Tracker.getFloorCenter().sub(Tracker.normalizedToFloor(corner1)).normalize().div(100)
						.rotate(-0.5f);
				PVector corner2Velocity = Tracker.getFloorCenter().sub(Tracker.normalizedToFloor(corner2)).normalize().div(100)
						.rotate(-0.5f);

				// ParticleImage(PVector loc, PVector velo, PVector accel, float rotation, float maxlife, int color, PImage img) {

				Particle particle1 = new ImageParticle(corner1, universe, img);
				particle1.velocity = corner1Velocity;
				particle1.acceleration = acceleration;
				particle1.color = goal1.color;

				Particle particle2 = new ImageParticle(corner2, universe, img);
				...
				Particle particleFluff = new ImageParticle(Particle.genRandomVector(Tracker.getFloorDimensionMax(), 0f, 0f), universe, img);
				particleFluff.acceleration = Particle.genRandomVector(0.01f / 300, 0.0f, 0.0f);
				particleFluff.velocity = Particle.genRandomVector(0.01f / 300, 0.0f, 0.0f);
				particleFluff.color = Color.WHITE.hashCode();
