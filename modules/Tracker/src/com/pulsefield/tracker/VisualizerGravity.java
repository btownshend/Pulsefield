...
public class VisualizerGravity extends Visualizer {
	...
	public void update(PApplet parent, People p) {
		...
		for (int i = 0; i < 10; i++) {
			// Give particles a dispersing force to mix them up a little (yes, we're re-using this but it's ok).
			PVector acceleration = Particle.genRandomVector(0.01f/300, 0.0f, 0.0f);

			// Send particles towards the black hole but with a rotation to avoid it.
			PVector corner1 = new PVector(1.0f, -1.0f);
			PVector corner2 = new PVector(-1.0f, 1.0f);
			PVector corner1Velocity = Tracker.getFloorCenter().sub(Tracker.normalizedToFloor(corner1)).div(200.0f).rotate(-0.3f);
			PVector corner2Velocity = Tracker.getFloorCenter().sub(Tracker.normalizedToFloor(corner2)).div(200.0f).rotate(-0.3f);
			...
			// Just some fluffy for more visual interest.
			universe.addParticle(Particle.genRandomVector(Math.max(Tracker.maxx, Tracker.maxy), 0.0f, 0.0f), Particle.genRandomVector(0.01f/300, 0.0f, 0.0f), Particle.genRandomVector(0.01f/300, 0.0f, 0.0f), 0xBBFFFFFF);
		}
		...
		for (int i = universe.particles.size()-1; i >= 0; i--) {
			...
			// Remove particles that have fallen outside the field.
			if ((particle.location.x > Tracker.maxx) ||
			    (particle.location.x < Tracker.minx) ||
			    (particle.location.y > Tracker.maxy) ||
			    (particle.location.y < Tracker.miny)) {
