...
if ~isfield(info,'grid')|| strcmp(op,'start')
  ...
  info.al.settempo(info.al.getsongtempo(info.grid.song));   % Set song tempo
...
for i=1:length(info.exits)
  ...
  if info.ableton
    info.al.stopsongtrack(info.grid.song,channel);
  end
...
nm=info.al.getsongclipname(info.grid.song,track,clip);
fprintf('cliptrigger: song=%d,track=%d (%s),clip=%d (%s),tempo=%d\n', info.grid.song,track, info.al.getsongtrackname(info.grid.song,track), clip, nm, info.al.getsongtempo(info.grid.song));
if isempty(nm)
  info.al.stopsongtrack(info.grid.song,track);
else
  info.al.playsongclip(info.grid.song,track,clip);
end
...
function updategriddisplay(p,info, prioractive)
  ...
  for i=scenes
    ...
    if isempty(ids)
      ...
      for j=1:length(ids)
        ...
        if info.al.haveclip(info.grid.song,channel,i)
  ...
  oscmsgout('TO','/grid/song',{info.al.getsongname(info.grid.song)});
