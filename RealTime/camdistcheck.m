...
idmap=[1,2,6,5,3,4];   % data is p.camera(i) is for physical camera idmap(i)
rmax=10;
r=0:.01:rmax;
r(2,:)=0;
rd=[];
...
for cam=1:1
  ...
  zfinal=fminsearch(@(z) nanmean(cerror(distdata,z(1:3),z(4:6),ledpos,pos).^2),zinit,options);
  ...
  err=sqrt(mean((dcalcphys(sel,1)-pos(sel,1)).^2+(dcalcphys(sel,2)-pos(sel,2)).^2));
  plot(distdata.cc(1),distdata.cc(2),'xr');
  legend('All LEDs','Expected','Optical Center','Location','SouthEast');
  title(sprintf('Raw pixel positions (RMS error=%.1f pixels)',err));
  ...
  summary(cam)=struct('campos',campos,'camdir',[camtheta,camphi,camroll],'ledpos',calcledpos,'phierr',phierror,'thetaerr',thetaerror,'pixerr',err);
end
